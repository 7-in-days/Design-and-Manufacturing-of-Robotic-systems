...
class OccupancyDWAPlanner:
    def __init__(self):
        ...
        #self.cruise_speed = 0.70 # 장애물이 멀거나 없을 때 기본 전진 속도 linear.x # 0.5 안정적 # Googi
        #self.avoid_speed = 0.30 # 장애물이 가까울 때 천천히 회피하면서 전진하는 속도  version Googi
        #self.stop_speed = 0.0 # 너무 가까운 장애물이 있거나 fallback일 때 정지 속도 version Googi
        
        
        self.cruise_speed = 1.2 # 장애물이 멀거나 없을 때 기본 전진 속도 linear.x # 0.5 안정적 # 스티커 없는거
        self.avoid_speed = 0.7# 장애물이 가까울 때 천천히 회피하면서 전진하는 속도  스티커 없는거
        self.stop_speed = 0.5 # 너무 가까운 장애물이 있거나 fallback일 때 정지 속도 version 스티커 없는거
        ...
        self.fov_deg = 180.0 # 전방 ROI 각도, 정면 기준 +/-70도 영역만 사용

        self.slow_cells = 33 # 가장 가까운 장애물이 이 cell보다 가까우면 avoid_speed로 감속
    ...
    def compute_command(self, grid, resolution, center_x, center_y):
        """occupancy grid를 받아 최종 linear.x, angular.z 명령을 계산한다."""
        ...
        if best_score < -1e17:
            fallback_w = self.choose_fallback_turn_direction(occ_rows, center_y)
    ...
    def get_front_obstacle_cells(self, grid, center_x, center_y):
        ...
        # Raw Go2 occupancy map front is -X, so front distance in cells is center_x - col.
        forward_cells = center_x - cols
    ...
    def nearest_obstacle_cells(self, occ_rows, occ_cols, center_x, center_y):
        """전방 ROI 장애물 중 로봇 중심과 가장 가까운 거리(cell 단위)를 계산한다."""
        if len(occ_rows) == 0:
            return 999.0

        forward_cells = center_x - occ_cols
    ...
    def select_linear_speed(self, nearest_cells):
        """가장 가까운 장애물 거리(cell)에 따라 고정 전진 속도를 선택한다."""
    ...
    def simulate_trajectory(self, v, wz):
        """주어진 linear.x와 angular.z로 미래 경로를 가상 시뮬레이션한다."""
    ...
    def score_trajectory_on_grid(self, traj, angular_z, occ_rows, occ_cols, resolution, center_x, center_y):
        """후보 경로 주변 장애물 cell 개수와 진행성을 이용해 DWA 점수를 계산한다."""
        if len(occ_rows) == 0:
    ...
    def forward_score(self, end_x, end_y, end_yaw, angular_z):
        """장애물 cost를 제외한 전진/방향/측면/회전 기본 점수를 계산한다."""
    ...
    def forward_xy_to_grid(x, y, resolution, center_x, center_y):
        """planner 기준 전방 x,y 좌표를 raw occupancy grid cell 좌표로 변환한다."""
        # Planner +X is robot forward. Raw occupancy map forward is -X.
    ...
    def choose_fallback_turn_direction(self, occ_rows, center_y):
        """모든 후보가 reject됐을 때 장애물이 적은 쪽으로 제자리 회전 방향을 정한다."""
        if len(occ_rows) == 0:
            return 0.0

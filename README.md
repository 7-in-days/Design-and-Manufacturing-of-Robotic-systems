...
Simulation은 NVIDIA Isaac Sim 기반 환경을 사용하였고, Real 환경에서는 실제 Go2와 노트북을
ROS2로 연결하여 주행하였다. `/occupancy_map` 토픽을 입력으로 받고, 계산된 속도 명령을
`/cmd_vel` 토픽으로 발행한다.
...
통해 장애물이 적은 방향을 다시 탐색하도록 처리하였다. 안전 경로가 하나 이상 존재할 때는 score가
가장 높은 경로를 선택하고, 안전 경로가 없을 때만 fallback으로 처리하였다.
...
| LiDAR Frame 처리 | Map에 따라 **Frame 누적** (Map1=10, Map2=3) | **Raw frame** 사용 | Sim에서는 Frame이 끊기는 현상 발생, Real에선 즉각 입력 |
| fallback 방식 | 방향 lock, 좌우 장애물 개수 판단 | 좌우 장애물 개수 판단 | Sim은 방향 진동 억제 필요, Real은 매 순간 반응 |
| Linear.x 처리 | 장애물 근접 시 **0.0**까지 감소 | **최소 전진 속도 유지** | Real은 정지 상태에서 회전 명령이 불안정 |
...
obstacle cell이 frame마다 불안정하게 검출되는 현상이 발생하였다. Rviz2로 확인한 결과, 로봇이 같은
위치에 정지해 있어도 LiDAR point 또는 occupancy map상의 장애물 cell이 깜빡이는 형태로 나타났다.
이로 인해 DWA는 매 주기마다 같은 장애물을 다르게 인식하였고, 회피 방향이 흔들리거나 fallback이
반복적으로 발생하였다.
...
Simulation에서 발생한 occupancy map의 불안정성을 줄이기 위해 LiDAR의 frame을 누적시켰다.
단일 frame의 obstacle cell만 사용하는 대신 최근 여러 frame의 occupancy map을 누적하여 장애물
cell을 유지하도록 하였다. **Map1에서는 10개, Map2에서는 3개**의 frame을 누적하였다.
...
<em>그림 4. Frame 누적 전 LiDAR Point (좌) · 그림 5. Frame 누적 후 LiDAR Point (우)</em>
</p>

Frame 누적 적용 후 동일 위치의 장애물이 frame마다 사라지지 않고 비교적 안정적으로 유지되었고,
이에 따라 DWA가 후보 경로를 평가할 때 장애물 위치를 더 일관되게 반영할 수 있었다. 반면 Real
환경에서는 raw frame을 누적할 경우 이미 지나간 장애물이 잔상처럼 남아 회피 판단에 영향을 주었으며,
raw frame으로 알고리즘이 잘 작동했기에 Real에서는 raw occupancy map을 사용하는 방식으로 구분하였다.
...
Real에 적용했을 때 어떤 차이가 발생하는지를 확인하는 데 있었다. 두 환경 모두 `/occupancy_map`을
입력으로 받아 DWA가 선택한 linear.x와 angular.z를 `/cmd_vel`로 발행하도록 구성하였고, cost 항목과
후보 경로 평가 방식도 동일하게 유지하였다.

그러나 실제 적용 과정에서는 두 환경의 차이가 분명하게 드러났다. Simulation에서는 occupancy map의
obstacle cell이 frame마다 불안정하게 검출되어 frame 누적이 필요했던 반면, Real에서는 raw frame을
그대로 사용하는 편이 더 적합하였다. 또한 두 환경의 occupancy map은 로봇 전방을 나타내는 x축 방향이
서로 달랐으며, 동일한 `/cmd_vel` 명령에 대해서도 플랫폼의 반응이 다르게 나타났다. 특히 Simulation에서는
linear.x = 0.0 상태에서도 회전이 가능했지만, 실제 Go2에서는 정지 상태에서의 회전이 안정적으로
수행되지 않았다.

이 경험을 통해 Simulation에서 잘 동작한 주행 코드가 Real에서도 동일한 결과를 보장하지는 않는다는
점을 확인할 수 있었다. 두 환경의 차이는 단순한 코드 구현상의 문제가 아니라, 센서 입력의 안정성,
좌표계 정의, actuator response, 통신 및 처리 지연, open-loop 구조의 한계가 복합적으로 결합된
Sim-to-Real Gap으로 볼 수 있다. 향후에는 `/cmd_vel`, IMU, odometry, joint state, occupancy map
update time을 함께 기록하여 system identification·simulator adaptation 방법으로 actuator model과
제어 파라미터에 반영하고, 현재의 DWA 기반 local planner는 유지하되 IMU·odometry를 결합한
closed-loop 구조로 확장하는 접근이 필요하다.
...
go2_real_final/
├── go2_final/
│   ├── go2_map1_controller.py   # Map1용 ROS2 노드 (Go2Control, Isaac_sim=True, 누적 10 frame)
│   ├── go2_map2_controller.py   # Map2용 ROS2 노드 (Go2Control, Isaac_sim=True, 누적 3 frame)
│   ├── go2_dwa_sim_map1.py      # Map1용 DWA planner (OccupancyDWAPlanner)
│   ├── go2_dwa_sim_map2.py      # Map2용 DWA planner (fallback 로직 차이)
│   ├── go2_sim_universal.py     # planner를 내장한 통합 sim 노드 (대안)
...
# Map 1
ros2 run go2_final go2_map1
# Map 2
ros2 run go2_final go2_map2
```
...
```mermaid
flowchart LR
    M1["go2_map1_controller<br/>(Go2Control node)"]
    M2["go2_map2_controller<br/>(Go2Control node)"]
    P1["go2_dwa_sim_map1<br/>OccupancyDWAPlanner"]
    P2["go2_dwa_sim_map2<br/>OccupancyDWAPlanner"]
    U["go2_sim_universal<br/>(planner 내장 통합 노드)"]
    DBG["occupancy_map_rviz_debug<br/>(RViz 시각화)"]

    M1 -- import --> P1
    M2 -- import --> P2
    DBG -. 동일 planner 재사용 .-> P1
    classDef node fill:#dbeafe,stroke:#2563eb;
    classDef plan fill:#dcfce7,stroke:#16a34a;
    class M1,M2,U,DBG node;
    class P1,P2 plan;
```

### 6.2 제어 루프 데이터 흐름 (sim)

```mermaid
flowchart TD
    OM(["/occupancy_map"]):::topic --> MC["map_callback()<br/>2D grid 재구성"]
    MC --> ACC["frame 누적<br/>logical_or, Map1=10·Map2=3"]
    ACC --> GRID["latest_grid<br/>resolution, center_x, center_y"]
    TIMER["10 Hz timer"] --> CL["control_loop()"]
    GRID --> CL
    CL --> CC["planner.compute_command()"]
    CC --> GF["get_front_obstacle_cells()<br/>ROI 전방 0.06~2.7m, 좌우 ±1.5m"]
    GF --> NO["nearest_obstacle_cells()"]
    NO --> SL["select_linear_speed()<br/>cruise / avoid / stop"]
    SL --> Z{"v ≈ 0 &<br/>장애물 존재?"}
    Z -- yes --> FR["get_locked_fallback_turn()<br/>방향 lock 회전"]
    Z -- no --> SAMP["w 후보 17개 sampling"]
    SAMP --> ST["simulate_trajectory()<br/>3초·30 step 예측"]
    ST --> SC["score_trajectory_on_grid()<br/>cost + hard constraint"]
    SC --> HC{"충돌 reject?"}
    HC -- 모두 reject --> FR
    HC -- 안전 경로 존재 --> BEST["best score 선택<br/>reset_fallback_lock()"]
    BEST --> OUT["(v, w)"]
    FR --> OUT
    OUT --> PUB["publish_cmd()"]
...
1. **입력 (`map_callback`)** — `/occupancy_map`(`nav_msgs/OccupancyGrid`)을 구독하여 400×400 2D grid로
   재구성한다. **Sim에서는** 최근 frame들의 이진 마스크(`grid >= 50`)를 `np.logical_or`로 누적해
   깜빡임을 억제한다(Map1=10, Map2=3 frame). 결과를 `latest_grid`에 저장한다.
2. **제어 루프 (`control_loop`, 10 Hz)** — map timeout(0.5 s)을 확인한 뒤 `planner.compute_command()`를
   호출하여 `(v, w, min_clearance, fallback_used, obstacle_count)`을 받는다.
3. **DWA 파이프라인 (`OccupancyDWAPlanner.compute_command`)**
   - `get_front_obstacle_cells()` — 전방 ROI 안의 장애물 cell만 추출 (Sim: `forward = cols − center_x`, **+x**).
   - `nearest_obstacle_cells()` → `select_linear_speed()` — 최근접 장애물 거리로 고정 선속도(cruise/avoid/stop) 결정.
   - 선속도가 0이고 장애물이 있으면 곧바로 `get_locked_fallback_turn()`으로 회전한다.
   - 그렇지 않으면 각속도 후보 17개를 sampling → 각 후보로 `simulate_trajectory()`(3초·30 step) →
     `score_trajectory_on_grid()`로 평가한다. 충돌 반경 안에 장애물이 있으면 **hard constraint**로 reject,
     남은 후보는 `progress/heading/lateral/angular/obstacle_count/close_distance` 가중치로 점수화한다.
   - 안전 후보가 있으면 최고 점수를 선택하고 `reset_fallback_lock()`, 전부 reject면 fallback 회전.
4. **출력 (`publish_cmd`)** — 선택된 `linear.x`, `angular.z`를 `/cmd_vel`로 발행한다.
5. **디버그 (`occupancy_map_rviz_debug`)** — 동일 planner로 모든 후보 경로/ROI/충돌 여부를 계산해
   `MarkerArray`로 RViz에 시각화한다.

> **Map1 vs Map2 차이**: Map2는 fallback 방향 결정 시 좌우 bias margin이 없고(대칭),
> 연속 clear frame을 세어 lock을 해제하는 `update_fallback_clear_lock()` 로직이 추가되어 있다.
> Frame 누적량도 Map1=10, Map2=3으로 다르다.
